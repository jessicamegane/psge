...
def read_data(path):
    ...
    if "aggregated_data.csv" in folders:
        ...
        for folder in folders:
            ...
            column_names = ['generation','best_fit','mean_fit','std_fit','best_test']
...
def boxplot(problem, df, metric, generation, display=False):

    bp_data = df[df['generation'] == generation]

    p = (ggplot(bp_data, aes(x = 'factor(algorithm)', y=metric)) 
         + geom_boxplot() 
         + scale_x_discrete(labels=['Dep Standard', 'Dep New Mut', 'Standard', 'New mut'], name='Algorithms')
         + labs(y='Error')
         + ggtitle(f'Boxplot of training data at generation {generation}'))
    
    p.save(filename = f'{problem}_performance_boxplot_train_{generation}.png')



def performance_plot(problem, df, display=True):
    ###### Performance - average
    mean_df = df.groupby(['generation','algorithm']).mean().reset_index()
    std_df = df.groupby(['generation','algorithm']).std().reset_index()

    # TODO: descobrir como pôr aqui o desvio padrão
    p = (ggplot(mean_df, aes(x='generation', y='best_fit', color='factor(algorithm)')) 
    + labs(x='Generations', y='Error')
    + scale_color_discrete(name = 'Algorithm version') # labels
    # + geom_ribbon(aes(ymin=mean_df['best_fit'] - std_df['best_fit'], ymax=mean_df['best_fit'] + std_df['best_fit']), alpha=0.2)
    + geom_line()
    + ggtitle(f'Performance for {problem} problem')
    )

    if display:
        print(p)

    p.save(filename = f'{problem}_performance_plot.png')

    boxplot(problem, df, 'best_fit', 250)
    
   
    if df['best_test'].sum() > 0:

        p = (ggplot(mean_df, aes(x='generation', y='best_test', color='factor(algorithm)')) 
        + labs(x='Generations', y='Error')
        + scale_color_discrete(name = 'Algorithm version')
        + geom_line()
        + ggtitle(f'Performance for {problem} problem - Test')
        )
        p.save(filename = f'{problem}_performance_plot_test.png')

        boxplot(problem, df, 'best_test', 250)

    # violin_data = df[df['generation'] == 100]
    # p = (ggplot(violin_data, aes(x = 'generation', y='best_fit')) + geom_violin(violin_data) + geom_point())

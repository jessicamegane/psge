...
class SymbolicRegression():
    def __init__(self, function="v3", has_test_set=True, invalid_fitness=9999999):
        self.__train_set = []
    ...
    def readpolynomial(self):
        ...
        def vladislavleva3(inp1, inp2):
            return _exp_(-inp1) * pow(inp1,3) * (cos(inp1) * sin(inp1)) * (cos(inp1) * pow(sin(inp1),2)) * (inp2 - 5)
        ...
        if self.function in ["pagiepolynomial"]:
            ...
            if self.has_test_set:
                ...
                self.test_set_size = len(self.__test_set)
        elif self.function in ["v3"]:
            l = []
            for xx in drange(0.05, 10, 0.1):
                for yy in drange(0.05, 10.05, 2):
                    zz = vladislavleva3(xx,yy)
                    l.append([xx,yy,zz])
            self.__train_set = l
            self.training_set_size = len(self.__train_set)
            if self.has_test_set:
                l = []
                for xx in drange(-0.5, 10.5, 0.05):
                    for yy in drange(-0.5, 10.5, 0.5):
                        zz = vladislavleva3(xx,yy)
                        l.append([xx,yy,zz])
                self.__test_set = l
                self.test_set_size = len(self.__test_set)
